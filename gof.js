...
function killOrborn(){
  ...
  let count; 
  for(let i = 1; i < maxColumn + 2; i++){
    for(let j = 1; j < maxRow + 2; j++){ 
...
function drawTable(){
  ...
  for(let i =canvasMaxColumn-79; i < canvasMaxColumn+1; i++){
    for(let j = canvasMaxRow-59; j < canvasMaxRow+1; j++){
	  if(table[j][i] == 1){
